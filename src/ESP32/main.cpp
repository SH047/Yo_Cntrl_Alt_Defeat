...
// ==== PIN Definitions ====
#define BUTTON_PIN 23  // Start button input
...
const float FRONT_STOP_CM = 25.0;
const float RIGHT_OPEN_CM = 90.0;
const float RIGHT_TURN_FRONT_CM = 75.0;
const float LEFT_TURN_FRONT_CM = 70.0;
const float RIGHT_TURN_TARGET_DEG = 117.0;
const float LEFT_TURN_TARGET_DEG = 133.0;

unsigned long lastStatusLog = 0;
const unsigned long STATUS_LOG_INTERVAL = 250;
...
void moveBackwardToSafeDistance() {
  ...
  float frontDist;
  do {
    driveBackward(REVERSE_SPEED); // Slow backward movement
    delay(100);
...
void turnLeft() {
  ...
  driveForward(DRIVE_SPEED); // Adjust speed as needed
...
void turnRight90Degrees() {
  ...
  driveForward(DRIVE_SPEED); // Slower speed for more precise turning
  ...
  while (millis() - turnStartTime < MAX_TURN_TIME && abs(angleChanged) < RIGHT_TURN_TARGET_DEG) {
    mpu6050.update();
    ...
    // Print progress at a controlled interval
...
void leftTurn() {
  ...
  // Keep turning until we've changed 85-90 degrees (account for some error)
  while (millis() - turnStartTime < MAX_TURN_TIME && abs(angleChanged) < LEFT_TURN_TARGET_DEG) {
    mpu6050.update();
    float currentYaw = getCompensatedYaw();
...
void setup() {
  ...
  straightenSteering(); // Set initial position to straight
...
void loop() {
  ...
  else if (distFront < RIGHT_TURN_FRONT_CM && distRight > RIGHT_OPEN_CM) {
      // Right turn
  ...
  else if (distFront < LEFT_TURN_FRONT_CM && distRight < RIGHT_OPEN_CM) {
      // Left turn
  ...
  else if (distRight > RIGHT_OPEN_CM) {
