...
const int DEFAULT_START_SPEED = 0;
const int STEERING_DEADBAND = 3;
...
void setMotor(int speed) {
  ...
  // Small command deadband prevents weak PWM commands from moving the motor.
  if (abs(speed) < 5) {
    speed = 0;
  }
...
void setSteering(int angle) {
  ...
  // Ignore very small changes to reduce unnecessary servo updates.
  if (abs(angle - currentSteering) >= STEERING_DEADBAND ||
      currentSteering == SERVO_STRAIGHT) {
...
void processLatestCommand() {
  ...
  if (cmd.length() > 0) {
    ...
    if (commaIndex > 0) {
      ...
      if (abs(speed) > (MAX_MOTOR_SPEED - 55) || abs(steering - SERVO_STRAIGHT) > 40) {
        Serial.print("ACK:");
        Serial.print(speed);
        Serial.print(",");
...
void loop() {
  ...
  // Communication safety: stop the drive motor if commands stop arriving.
  if (lastCommandTime > 0 &&
      currentTime - lastCommandTime > COMMAND_TIMEOUT) {
    if (currentSpeed != 0) {
      setMotor(0);
      Serial.println("SAFETY_STOP");
    }
  }
